...
def convert_day_to_week(df, day_column='day', week_column='week'):
    ...
    df[week_column] = df[week_column].astype('category')
...
def convert_month_to_quarter(df, month_column='month', quarter_column='quarter', 
    ...
    df[quarter_column] = pd.cut(df['month_num'], bins=[0, 3, 6, 9, 12], 
                                labels=[1, 2, 3, 4])
    df[quarter_column] = df[quarter_column].astype('category')
...
def fit_model(model, preprocessor, X_train, y_train):
    """
    Build a pipeline with the given preprocessor + model and fit it.
...
def plot_roc_auc(y_true, y_proba, title="ROC Curve"):
    """
    Plot ROC curve and print AUC.
    Requires probabilities for the positive class.
    """
    if y_proba is None:
        raise ValueError("y_proba is None – this model does not support predict_proba.")
...
def calculate_auc_score(pipeline, X_test, y_test):
    """
    Calculate and return the ROC-AUC score for a fitted pipeline.
    Works with any classifier that supports predict_proba or decision_function.
    """
    # Prefer predict_proba if available
    if hasattr(pipeline, "predict_proba"):
        y_proba = pipeline.predict_proba(X_test)[:, 1]
    # Otherwise use decision_function (e.g., SVM)
    elif hasattr(pipeline, "decision_function"):
        y_proba = pipeline.decision_function(X_test)
    else:
        raise ValueError("Model does not support probability or decision scores for ROC-AUC.")
